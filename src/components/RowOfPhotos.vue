<template>
	<div :set="[shown_photo_count=row_photos.filter(photo => !excluded_cameras.includes(photo.camera_model)).length, total_photo_count=row_photos.length]">
		<div v-if="!shown_photo_count && !total_photo_count" class="info_message"><font-awesome-icon icon="info-circle" /> no imported photos fall within the time range of this trail </div>
